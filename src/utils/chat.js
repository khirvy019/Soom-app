...
export function encryptMessage(opts={ data: '', privkey: '', pubkeys: '' }) {
  ...
  if (pks.length === 1) {
    ...
    return { encrypted, authorPubkey: ourPubkey, pubkeys: pks, iv: Buffer.from(iv).toString('base64') }
  } else {
    ...
    return { encrypted, authorPubkey: ourPubkey, pubkeys, iv: Buffer.from(iv).toString('base64') }
  }
}
...
export function decryptMessage(opts={data: '', iv: '', privkey: '', authorPubkey: '', pubkeys: []}) {
  ...
  for(var i = 0; i < pubkeys.length; i++) {
    try {
      ...
      if (pubkey == ourPubkey) {
        console.log('Using author pubkey')
        pubkey = authorPubkey
      }
      const ourPriv = secp.etc.hexToBytes(privkey)
      const otherPub = secp.etc.hexToBytes(pubkey)
